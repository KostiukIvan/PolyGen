...
class VertexTokenizer:
    def __init__(self, max_seq_len=810):
    ...
    def get_initial_sampling_tokens(self, num_samples):
        """
        ...
        """
        vertices_tokens = axises_tokens = position_tokens = torch.tensor([self.tokens['bos'][0]])
        return {
            key: val.expand(num_samples, - 1)
            for key, val in self.padding(vertices_tokens, axises_tokens, position_tokens).items()
        }
